...
export default function ProductPage() {
    ...
    const [formData, setFormData] = useState({ name: '', product_group_id: '', factory_id: '', is_active: true });

    // Queries
    const { data: factories } = useQuery({ queryKey: ['factories'], queryFn: factoryService.getAll });
    const { data: productGroups } = useQuery({ queryKey: ['productGroups', filterFactoryId], queryFn: () => productGroupService.getAll(filterFactoryId) });
    ...
    const handleOpen = (product = null) => {
        if (product) {
            ...
            setFormData({ name: '', product_group_id: '', factory_id: '', is_active: true });
        }
        setOpenModal(true);
    };
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (selectedProduct) updateMutation.mutate({ id: selectedProduct.id, payload: formData });
    ...
    // Filter product groups by currently selected factory in the form
    const availableGroupsInForm = productGroups?.filter(g => g.factory_id === formData.factory_id);
                    ...
                    <Box display="flex" justifyContent="space-between" alignItems="center" gap={2}>
                        ...
                        <Box display="flex" gap={2}>
                            <FormControl size="small" sx={{ minWidth: 200 }}>
                                ...
                                <Select value={filterFactoryId} label="Lọc theo nhà máy" onChange={(e) => setFilterFactoryId(e.target.value)}>
                                    <MenuItem value=""><em>Tất cả nhà máy</em></MenuItem>
                                    {factories?.map(f => <MenuItem key={f.id} value={f.id}>{f.name}</MenuItem>)}
                                </Select>
                            </FormControl>
                            <Button variant="contained" color="secondary" onClick={() => handleOpen()}>+ Thêm mã hàng</Button>
                        </Box>
                    </Box>
                }
                data={products}
                    ...
                    <DialogContent dividers>
                        <FormControl fullWidth margin="normal" required>
                            ...
                            <Select 
                                ...
                                label="Nhà máy" 
                                onChange={(e) => setFormData({...formData, factory_id: e.target.value, product_group_id: ''})} // Reset group on factory change
                            >
                                {factories?.filter(f=>f.is_active).map(f => <MenuItem key={f.id} value={f.id}>{f.name}</MenuItem>)}
                            </Select>
                        </FormControl>

                        <FormControl fullWidth margin="normal" required disabled={!formData.factory_id}>
                            ...
                            >
                                {availableGroupsInForm?.map(g => <MenuItem key={g.id} value={g.id}>{g.name}</MenuItem>)}
                            </Select>
                        </FormControl>

                        <TextField fullWidth label="Tên sản phẩm" margin="normal" required value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} />
