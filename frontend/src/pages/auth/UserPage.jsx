...
import { 
    Checkbox, Paper, Chip, Typography, List, ListItem, ListItemText, Divider,Button,Dialog,DialogTitle,DialogContent,DialogActions,TextField,Select,MenuItem,InputLabel,FormControl,FormControlLabel,Switch,
    Table, TableHead, TableBody, TableRow, TableCell, TableContainer, Tooltip, IconButton, Box
} from '@mui/material';
...
export default function UserPage() {
    ...
    const handleDeleteRole = (role) => {
        ...
        if (window.confirm(`Xóa vai trò "${role.name}"? Tất cả người dùng thuộc vai trò này sẽ được chuyển về "DEFAULT_USER".`)) {
        ...
        <Box>
             <GenericTable 
                title={
                    ...
                    <Box display="flex" justifyContent="space-between" alignItems="center" gap={2}>
                        <Button variant="outlined" color="primary" onClick={() => setOpenRoleModal(true)}>
                            Quản lý Vai trò
                        </Button>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <DialogContent dividers>
                        <Box display="grid" gridTemplateColumns="1fr 1fr" gap={2}>
                            <Box>
                                <Typography variant="subtitle2" gutterBottom fontWeight={600} color="primary">Thông tin cá nhân</Typography>
                                <TextField 
                                    fullWidth label="Tên hiển thị (Họ tên)" size="small" margin="dense"
                                    value={formData.full_name} 
                                    onChange={(e) => setFormData({...formData, full_name: e.target.value})} 
                                />
                                <Box display="grid" gridTemplateColumns="1fr 1fr" gap={1}>
                                    <TextField 
                                        fullWidth label="Số điện thoại" size="small" margin="dense"
                                        value={formData.phone} 
                                        onChange={(e) => setFormData({...formData, phone: e.target.value})} 
                                    />
                                    <TextField 
                                        fullWidth label="Email" size="small" margin="dense"
                                        value={formData.email} 
                                        onChange={(e) => setFormData({...formData, email: e.target.value})} 
                                    />
                                </Box>

                                <Typography variant="subtitle2" gutterBottom fontWeight={600} color="primary" sx={{ mt: 2 }}>Tài khoản & Hệ thống</Typography>
                                <TextField 
            ...
            >
                ...
                <DialogContent sx={{ p: 0 }}>
                    ...
                    <Box sx={{ p: 3, bgcolor: 'background.default', borderBottom: '1px solid', borderColor: 'divider' }}>
                        ...
                        <Box display="flex" gap={2} mt={1.5}>
                            ...
                            <Button 
                                ...
                                sx={{ borderRadius: '10px', px: 4, fontWeight: 700, boxShadow: 'none' }}
                            >
                    ...
                    <Box sx={{ p: 0 }}>
                        <TableContainer sx={{ maxHeight: 350 }}>
                            <Table stickyHeader size="small">
                                <TableHead>
                                    <TableRow>
                                        <TableCell sx={{ fontWeight: 700, bgcolor: 'background.paper', py: 1.5 }}>VAI TRÒ</TableCell>
                                        <TableCell sx={{ fontWeight: 700, bgcolor: 'background.paper' }}>LOẠI</TableCell>
                                        <TableCell align="right" sx={{ fontWeight: 700, bgcolor: 'background.paper' }}>THAO TÁC</TableCell>
                                    </TableRow>
                                </TableHead>
                                <TableBody>
                                    {roles?.map((role) => (
                                            ...
                                            <TableCell>
                                                <Chip 
                                                    ...
                                                    sx={{ 
                                                        fontSize: '0.7rem', 
                                                        height: 20, 
                                                        bgcolor: role.is_system ? 'rgba(51, 65, 85, 0.08)' : 'transparent',
                                                        borderColor: role.is_system ? 'transparent' : 'divider',
                                                        fontWeight: 600
                                                    }} 
                                                />
                                            </TableCell>
                    ...
                    {/* Bottom Warning */}
                    <Box sx={{ px: 3, py: 2, bgcolor: 'rgba(234, 88, 12, 0.02)', borderTop: '1px solid', borderColor: 'divider' }}>
                        <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start' }}>
                            <WarningAmberIcon sx={{ color: '#ea580c', fontSize: '1.2rem', mt: 0.1 }} />
                            <Typography variant="caption" sx={{ color: '#9a3412', fontWeight: 500, lineHeight: 1.5 }}>
                                Khi xóa vai trò tùy chỉnh, tất cả người dùng thuộc vai trò đó sẽ tự động chuyển về <strong>DEFAULT_USER</strong> (không có quyền truy cập mặc định).
                            </Typography>
                        </Box>
                    </Box>
                </DialogContent>
