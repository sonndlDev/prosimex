  ...
  const allowedMenus = menuItems.filter((item, index) => {
    if (item.type === "subheader") {
      ...
      const itemsInSection =
        firstSubheader === -1 ? nextItems : nextItems.slice(0, firstSubheader);
      ...
      >
        <Toolbar sx={{ justifyContent: "space-between" }}>
          <Box display="flex" alignItems="center">
            ...
            >
              {allowedMenus.find((m) => m.path === location.pathname)?.text ||
                "Hệ thống điều hành"}
            </Typography>
          </Box>
