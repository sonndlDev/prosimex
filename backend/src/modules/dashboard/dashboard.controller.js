...
export const getMetrics = async (req, res) => {
  try {
    ...
    // 5. Today's attendance (overall stats)
    ...
    `);
    const totalWorkersRes = await pool.query(
    ...
    // 5b. Current user's attendance status today
    const myAttendanceRes = await pool.query(
    ...
    // 6. Urgent orders (delivery_date within 7 days, not DONE/CANCELLED)
    const urgentOrdersRes = await pool.query(`
      SELECT 
    ...
    // 7. Production progress (IN_PROGRESS orders)
    const productionProgressRes = await pool.query(`
      SELECT 
        o.id,
        o.order_code,
        ...
        COALESCE(
          (SELECT SUM(dti.actual_quantity) 
           ...
           WHERE dti.order_id = o.id AND dt.status = 'COMPLETED' AND dt.deleted_at IS NULL),
          0
